...
def FindReversal(stockMA, reversalDay=8):
    ...
    for stockMAKey, stockMAValue in stockMA.items():
        ...
        if float(stockMAValue["ma10"]) <= float(stockMAValue["ma20"]):
            ...
            compareCount += 1
        # 遇到死叉本轮趋势终止，若本轮下跌趋势满足趋势时长，在OrderDic添加本轮趋势信息（默认不记录死叉当天信息，若记录死叉当天信息，在此处添加即可）
        if float(stockMAValue["ma10"]) > float(stockMAValue["ma20"]):
            if compareCount > reversalDay:
                findIndex = np.array(seriesList)[:,0].tolist()
                findIndex.reverse()
                min_index = findIndex.index(min(findIndex))
                findIndex.reverse()
                theIndex = len(findIndex)-min_index-1
                #print(theIndex, findIndex[theIndex])
                #print(seriesTdateList[:theIndex+1])
                #print(seriesList[:theIndex+1])
                OrderDic[stockMAValue["tdate"]] = {"tdateList":seriesTdateList[:theIndex+1], "seriesList":seriesList[:theIndex+1]}
                # to do: 寻找区间最低点，记录最低点之前的日期序列用于作图
                seriesTdateList = list()
                seriesList = list()
            compareCount = 0
    return OrderDic
...
if __name__ == '__main__':
    ...
    if len(OrderDic[listIndex[0]]["tdateList"]) > reversalDay - 5:
        DrawHelper.DrawDataK(OrderDic[listIndex[0]]["tdateList"][-reversalDay+5:], OrderDic[listIndex[0]]["seriesList"][-reversalDay+5:])
    else:
        pass
