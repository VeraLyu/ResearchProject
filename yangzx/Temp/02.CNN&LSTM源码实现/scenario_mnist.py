...
# 训练数据
path_minst_unpack = 'E:\MyGit\BigDataFile\MNIST_data_unpack'
...
class Params:
    ...
    IMAGE_SIZE = 28
    # MINST通道数为1，也是第一个卷积块的输入通道，可支持多通道
    IMAGE_CHANNEL = 1  
...
class MnistData(object):
    ...
    def __init__(self, absPath, is4Cnn, dataType):
        ...
        self.sample_range = [i for i in range(len(self.labels))]
        self.sample_range_v = [i for i in range(len(self.labels_v))]
...
def main():
    ...
    conv1 = ConvLayer('conv1', Params.MINI_BATCH_SIZE, Params.IMAGE_SIZE, Params.IMAGE_CHANNEL,
                      Params.CONV1_F_SIZE, Params.CONV1_O_DEPTH, Params.CONV1_O_SIZE, Params.CONV1_STRIDES,
                      NoAct, AdamOptimizer,optmParamsAdam, Params.DTYPE_DEFAULT,Params.INIT_W)
    #在BN后再激活
    bn1 = BNLayer('bn1',Params.EPS,Params.MINI_BATCH_SIZE,Params.CONV1_O_DEPTH, Params.CONV1_O_SIZE,ReLU,AdamOptimizer,optmParamsAdam, Params.DTYPE_DEFAULT)
    pool1 = MaxPoolLayer('pool1', Params.MINI_BATCH_SIZE, Params.POOL1_F_SIZE, Params.POOL1_STRIDES, False, Params.DTYPE_DEFAULT)

    # 在Conv2和relu/pool2之间加入bn2,conv2的输出不再激活
    conv2 = ConvLayer('conv2', Params.MINI_BATCH_SIZE, Params.CONV2_O_SIZE, Params.CONV1_O_DEPTH, Params.CONV2_F_SIZE, Params.CONV2_O_DEPTH, Params.CONV2_O_SIZE, Params.CONV2_STRIDES, NoAct, AdamOptimizer,optmParamsAdam, Params.DTYPE_DEFAULT,Params.INIT_W)
    bn2 = BNLayer('bn2',Params.EPS,Params.MINI_BATCH_SIZE,Params.CONV2_O_DEPTH, Params.CONV2_O_SIZE,ReLU,AdamOptimizer,optmParamsAdam, Params.DTYPE_DEFAULT)
    pool2 = MaxPoolLayer('pool2', Params.MINI_BATCH_SIZE, Params.POOL2_F_SIZE, Params.POOL2_STRIDES, True, Params.DTYPE_DEFAULT)

    fc1 = FCLayer(Params.MINI_BATCH_SIZE, Params.FC1_SIZE_INPUT, Params.FC1_SIZE_OUTPUT, ReLU, AdamOptimizer, optmParamsAdam,False, Params.DTYPE_DEFAULT,Params.INIT_W)
    fc2 = FCLayer(Params.MINI_BATCH_SIZE, Params.FC1_SIZE_OUTPUT, Params.TYPE_K, NoAct, AdamOptimizer, optmParamsAdam,False, Params.DTYPE_DEFAULT,Params.INIT_W)
